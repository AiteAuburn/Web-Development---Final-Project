...
public class GigOrderService extends Service {
  public int submitReview(int uid, String orderId, String rt, String comment) {
    int errorCode = 0;
    int oid = 0;
    int ratings = 0;
    if (uid > 0) {
      ...
      try {
        ...
        preparedStatement = connect.prepareStatement(
            "SELECT o.oid, o.worker_uid,  o.requester_uid, o.status FROM orders o WHERE o.oid = ? AND (o.worker_uid = ? OR o.requester_uid = ?)");
        ...
        if (orderExist) {
          ...
          if (!status.equalsIgnoreCase("e")) {
            ...
            if (canReview) {
              preparedStatement = connect.prepareStatement(
                  "INSERT INTO comment(oid, from_uid, to_uid, ratings, comment) VALUES(?, ?, ?, ?, ?)");
              preparedStatement.setInt(1, oid);
  ...
  public OrderModel getOrder(int uid, int oid) {
    ...
    if (uid > 0) {
      try {
        ...
        statement = connect.createStatement();
        preparedStatement = connect.prepareStatement(
            "SELECT o.oid, o.worker_uid,  CONCAT(u1.fname,' ', u1.lname) as worker_name, o.requester_uid, CONCAT(u2.fname,' ', u2.lname) as requester_name, o.title, o.location, o.description, o.price, o.status, o.create_time "
                + "FROM orders o " + "LEFT JOIN user u1 " + "ON o.worker_uid = u1.uid " + "LEFT JOIN user u2 "
                + "ON o.requester_uid = u2.uid " + "WHERE o.oid = ? " + "AND (o.requester_uid = ? OR o.worker_uid = ?) "
                + "LIMIT 1");
        preparedStatement.setInt(1, oid);
        preparedStatement.setInt(2, uid);
        preparedStatement.setInt(3, uid);
        ...
        if (result) {
          ...
          preparedStatement = connect.prepareStatement(
              "SELECT ratings, comment, create_time FROM comment WHERE oid = ? AND from_uid = ? LIMIT 1");
          preparedStatement.setInt(1, oid);
  ...
  public ArrayList<OrderModel> getOrderList(int uid) {
