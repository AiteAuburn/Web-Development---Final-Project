...
public class GigWorkerService extends Service {
  ...
  public int acceptRequest(int uid, String requestId) {
    int errorCode = 0;
    int rid = 0;
    if (uid > 0) {
      try {
      ...
      try {
        ...
        preparedStatement = connect.prepareStatement(
            "SELECT s.title, r.description, r.request_uid, r.location, r.price FROM request r LEFT JOIN service s ON r.sid = s.sid WHERE s.uid = ? AND r.rid = ? AND r.status = 'o' AND s.enabled = 1");
  ...
  public int rejectRequest(int uid, String requestId) {
    int errorCode = 0;
    int rid = 0;
    if (uid > 0) {
      try {
      ...
      try {
        ...
        statement = connect.createStatement();
        preparedStatement = connect.prepareStatement(
            "SELECT rid FROM request r LEFT JOIN service s ON r.sid = s.sid WHERE s.uid = ? AND rid = ?");
        preparedStatement.setInt(1, uid);
        preparedStatement.setInt(2, rid);
        resultSet = preparedStatement.executeQuery();
        boolean requestExist = resultSet.next();
  ...
  public ArrayList<RequestModel> getRequestList(int uid, int tid) {
    ...
    if (uid > 0) {
      try {
        ...
        statement = connect.createStatement();
        preparedStatement = connect.prepareStatement(
            "SELECT rid, fname, lname, r.price, r.create_time FROM service s LEFT JOIN request r ON s.sid = r.sid LEFT JOIN user u ON r.request_uid = u.uid WHERE s.sid = ? AND s.uid = ? AND r.status ='o'");
  ...
  public int requestWorker(int uid, String serviceId, String location, String description) {
    int errorCode = 0;
    int sid = 0;
    if (uid > 0) {
      ...
      try {
        ...
        if (!serviceExist) {
          ...
          preparedStatement = connect
              .prepareStatement("SELECT rid from request WHERE sid = ? AND request_uid = ? LIMIT 1");
          preparedStatement.setInt(1, sid);
          preparedStatement.setInt(2, uid);
          resultSet = preparedStatement.executeQuery();
          boolean requestExist = resultSet.next();
          if (requestExist) {
            ...
            preparedStatement = connect.prepareStatement(
                "UPDATE request SET location = ?, description = ?, price = ?, status = 'o' WHERE rid = ?");
            ...
            preparedStatement = connect.prepareStatement(
                "INSERT INTO request (sid, request_uid, location, description, price) VALUES (?, ?, ?, ?, ?)");
            preparedStatement.setInt(1, sid);
            preparedStatement.setInt(2, uid);
  ...
  public int cancelRequest(int uid, String serviceId) {
    int errorCode = 0;
    int sid = 0;
    if (uid > 0) {
      if (serviceId == null || serviceId.length() == 0)
  ...
  public WorkerModel getWorker(int uid, int sid) {
    ...
    if (uid > 0) {
      try {
        ...
        statement = connect.createStatement();
        preparedStatement = connect.prepareStatement(
            "SELECT s.uid, r.status as requestStatus, r.location as requestLocation, r.description as requestDescription, rid, CONCAT(fname, ' ', lname) as name, title, s.price, s.description, enabled, SUM(ratings)/COUNT(ratings) as ratings from service s "
                + "LEFT JOIN comment c " + "ON c.to_uid = s.uid " + "LEFT JOIN user u " + "ON s.uid = u.uid "
                + "LEFT JOIN request r " + "ON r.request_uid = ? AND r.sid = s.sid " + "WHERE s.sid = ? LIMIT 1");
        preparedStatement.setInt(1, uid);
  ...
  public ArrayList<WorkerModel> getWorkerList() {
    ...
    try {
      ...
      statement = connect.createStatement();
      preparedStatement = connect.prepareStatement(
          "SELECT sid, CONCAT(fname, ' ', lname) as name, title, price, description, enabled, SUM(ratings)/COUNT(ratings) as ratings FROM service s, user u LEFT JOIN comment c ON c.to_uid = u.uid WHERE u.uid = s.uid AND enabled = 1 GROUP BY u.uid ORDER BY s.sid DESC");
      resultSet = preparedStatement.executeQuery();
      while (resultSet.next()) {
  ...
  public int editService(int uid, String title, String price, String description, String enabled) {
    ...
    try {
      ...
      if (recordExist) {
        ...
        preparedStatement = connect
            .prepareStatement("UPDATE service SET title = ?, price = ?, description = ?, enabled = ? WHERE uid = ?");
        ...
        preparedStatement = connect
            .prepareStatement("INSERT INTO service (uid, title, price, description, enabled) VALUES (?, ?, ?, ?, ?)");
        preparedStatement.setInt(1, uid);
