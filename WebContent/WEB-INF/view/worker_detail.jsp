...
<%@ page import="aite.model.RequestModel"%>
			...
			<div class="worker_detail">
				<div class="header row">
					...
					<div class="info">
						...
						<div class="ratings">
							<span class="star">&#x2605</span><%= service.ratings %></div>
					</div>
				</div>
				...
				<div class="ratings row">
					...
					<h3 class="score">
						<a
							href="${pageContext.request.contextPath}/user/comments?uid=<%=service.uid%>"><%= service.ratings %>
							&rarr;</a>
					</h3>
				</div>
				<% 
	          ...
	          %>
				<% if ( service.rid != 0 && service.requestStatus.equalsIgnoreCase("o") ) { %>
					<form
						action="${pageContext.request.contextPath}/gigworkers/request_cancel"
						method="post">
						<h1 class="page-title">Your Request</h1>
						<h5 class="label">
							Price: $<%= service.price %></h5>
						<h5 class="label">
							Location:
							<%= service.requestLocation %></h5>
						<h5 class="label">Description</h5>
						<pre><%= service.requestDescription %></pre>
						<input type="hidden" name="sid" value="<%= service.sid%>" /> <input
							type="submit" value="Cancel Request" />
					</form>
				<% } else if ( uid != service.uid ) { %>
					<form action="${pageContext.request.contextPath}/gigworkers/request"
						method="post">
						...
						<input type="submit"
							value="Send Request ($<%= service.price %> / One Time)" />
					</form>
				<% } else { %>
